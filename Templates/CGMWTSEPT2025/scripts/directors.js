...
function createDirectorsHTML() {
  ...
  directors.forEach((director) => {
    ...
    const previewSrc = resolveAssetPath(director.previewImg);
    const directorRoute = resolveRoutePath(director.route);

    directorElement.innerHTML = `
      <a href="${directorRoute}" class="director-link">
        <div class="director-name">
          <h2>${director.name}</h2>
        </div>
        <div class="director-preview">
          <img src="${previewSrc}" alt="${director.name}" />
        </div>
      </a>
    `;
