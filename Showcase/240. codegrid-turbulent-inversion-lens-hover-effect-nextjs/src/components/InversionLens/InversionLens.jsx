...
const InversionLens = ({ src, className }) => {
  ...
  const resolvedSrc = useMemo(() => withAssetPrefix(src), [src]);

  useEffect(() => {
    if (isSetupCompleteRef.current || !containerRef.current || !resolvedSrc)
      return;

    const loader = new THREE.TextureLoader();

    const loadTexture = () => {
      loader.load(resolvedSrc, (texture) => {
        setupScene(texture);
        setupEventListeners();
        animate();
  ...
  return (
    <div ref={containerRef} className={`inversion-lens ${className || ""}`}>
      <img src={resolvedSrc} alt="" style={{ display: "none" }} />
    </div>
  );
};
